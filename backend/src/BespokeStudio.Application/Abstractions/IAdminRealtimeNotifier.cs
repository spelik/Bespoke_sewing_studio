...
public interface IAdminRealtimeNotifier
{
    ...
    Task NotifyOrderDeletedAsync(Guid orderId, string? referenceNumber, CancellationToken cancellationToken = default);
    ...
    Task NotifyContactMessageDeletedAsync(Guid contactMessageId, string? referenceNumber, CancellationToken cancellationToken = default);
