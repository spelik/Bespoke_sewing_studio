...
public static class BrandSettingsEndpoints
{
    ...
    {
        endpoints.MapGet("/api/brand-settings/public",async(ISiteSettingsService s,CancellationToken ct)=>TypedResults.Ok(await s.GetPublicBrandSettingsAsync(ct))).AllowAnonymous().CachePublicContent(PublicOutputCachePolicy.BrandSettingsTag).WithTags("Brand Settings").WithName("GetPublicBrandSettings").Produces<PublicBrandSettingsResponse>();
    ...
    private static async Task<IResult> UpdateAsync(UpdateBrandSettingsRequest r,ClaimsPrincipal principal,ISiteSettingsService s,IAdminAuditLogService auditLogService,IOutputCacheStore outputCacheStore,CancellationToken ct)
    { var errors=BrandSettingsValidator.Validate(r); if(errors.Count>0)return Validation(errors); try{var settings=await s.UpdateBrandSettingsAsync(r,ct); await auditLogService.RecordAsync(AdminAuditEndpointHelpers.CreateAuditRequest(principal,"brand_settings.updated","BrandSettings",null,settings.BrandDisplayName,"Brand / SEO settings were updated."),ct); await PublicOutputCacheInvalidation.EvictAsync(outputCacheStore,ct,PublicOutputCachePolicy.BrandSettingsTag); return TypedResults.Ok(settings);}catch(BrandSettingsConflictException ex){return Problem(ex.Field,ex.Message);} }
