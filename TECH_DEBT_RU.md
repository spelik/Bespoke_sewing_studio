...
- Для Email Outbox остаются future-задачи: external alerting/notifications (email/Slack/webhook) по exhausted jobs, retention/очистка завершённых outbox bodies, multi-instance locking hardening и PostgreSQL integration tests. (Manual retry реализован — см. Task 76; базовый read-only admin monitoring на Dashboard/Email Log реализован — см. Task 77.)
...
- После Task 65 остаются tag-based invalidation после admin CMS mutations, production CDN/reverse-proxy cache headers/strategy и cache hit/miss metrics. Пока public CMS может обновляться с задержкой до 60 секунд.
