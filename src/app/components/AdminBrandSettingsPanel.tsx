...
type UploadKey = "logoFileId" | "faviconFileId" | "defaultOgImageFileId";

const input =
  "w-full px-3 py-2.5 text-[12px] border border-border bg-background focus:outline-none focus:border-accent";
const empty: UpdateBrandSettingsRequest = {
  brandDisplayName: "",
  logoFileId: null,
  logoAltText: "",
  faviconFileId: null,
...
const requestFrom = (s: AdminBrandSettings): UpdateBrandSettingsRequest => ({
  ...
  defaultOgImageFileId: s.defaultOgImageFileId,
  ...s.navigation,
});
...
export function AdminBrandSettingsPanel({ onUnauthorized }: Props) {
  ...
  const [current, setCurrent] = useState<AdminBrandSettings | null>(null);
  const [busy, setBusy] = useState(true);
  const [activeUploadKey, setActiveUploadKey] = useState<UploadKey | null>(null);
  const [uploadQueue, setUploadQueue] = useState<UploadQueueState>(createEmptyUploadQueue());
  const [lastFile, setLastFile] = useState<{ key: UploadKey; file: File } | null>(null);
  ...
  function set<K extends keyof UpdateBrandSettingsRequest>(
    key: K,
  ...
  async function upload(key: UploadKey, file?: File) {
      ...
      [
        {
          ...
          upload: ({ onProgress, signal }) =>
            uploadBrandImage(file, { onProgress, signal }),
        },
      ],
      { signal: controller.signal, onQueueChange: setUploadQueue },
    );
    ...
    if (outcome.failures.length > 0) {
      handle(outcome.failures[0]?.error);
      return;
    }
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      const x = await updateAdminBrandSettings(form);
      setCurrent(x);
      setForm(requestFrom(x));
      await refresh();
      setMessage("Brand and SEO settings saved.");
    } catch (e) {
      handle(e);
    } finally {
      setBusy(false);
  ...
  if (busy && !current) {
    return <p className="text-[12px] text-muted-foreground">Loading brand settings...</p>;
  }

  const uploadBusy = activeUploadKey !== null;
  const uploadBox = (
    label: string,
    key: UploadKey,
    url: string | null,
    ...
    <div>
      ...
      <div className="mt-2 flex flex-col gap-3">
        {previews[key] || url ? (
          <img
            src={previews[key] || url || ""}
            alt="Preview"
            className="h-16 w-28 object-contain border border-border bg-white p-2"
          />
        ) : (
          <div className="h-16 w-28 border border-dashed border-border flex items-center justify-center text-[10px] text-muted-foreground">
            No image
          </div>
        )}
        <UploadProgressControl
          idleLabel="Choose image"
          disabled={uploadBusy && activeUploadKey !== key}
          item={activeUploadKey === key ? uploadQueue.items[0] ?? null : null}
          onFilesSelected={(files) => void upload(key, files[0])}
          onRetry={
            lastFile?.key === key
              ? () => void upload(key, lastFile.file)
              : undefined
          }
        />
        {form[key] ? (
  ...
  return (
    <div className="space-y-6">
      {error ? (
        <div role="alert" className="border border-destructive/30 bg-card p-3 text-[11px] text-destructive">
          {error}
        </div>
      ) : null}
      {message ? (
        <div className="border border-emerald-300 bg-card p-3 text-[11px] text-emerald-700">
          {message}
        </div>
      ) : null}
      <section className="bg-card border border-border p-6 space-y-5">
        <h2 className="font-serif text-xl">Brand assets</h2>
        <div className="grid md:grid-cols-3 gap-6">
          {uploadBox("Header / footer logo", "logoFileId", current?.logoUrl ?? null)}
          {uploadBox("Favicon", "faviconFileId", current?.faviconUrl ?? null)}
          {uploadBox(
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Field label="Brand display name" value={form.brandDisplayName} onChange={(v) => set("brandDisplayName", v)} />
          <Field label="Logo alt text" value={form.logoAltText} onChange={(v) => set("logoAltText", v)} />
          <Field label="Header CTA label" value={form.headerCtaLabel} onChange={(v) => set("headerCtaLabel", v)} />
          <Field label="Header CTA URL" value={form.headerCtaUrl} onChange={(v) => set("headerCtaUrl", v)} />
        </div>
      </section>
      <section className="bg-card border border-border p-6 space-y-4">
        <h2 className="font-serif text-xl">Default SEO</h2>
        <Field label="Meta title" value={form.defaultMetaTitle} onChange={(v) => set("defaultMetaTitle", v)} />
        <Area label="Meta description" value={form.defaultMetaDescription} onChange={(v) => set("defaultMetaDescription", v)} />
        <div className="grid md:grid-cols-2 gap-4">
          <Field
            label="Open Graph title (optional)"
            value={form.defaultOgTitle ?? ""}
            onChange={(v) => set("defaultOgTitle", v || null)}
          />
          <Area
            label="Open Graph description (optional)"
            value={form.defaultOgDescription ?? ""}
            onChange={(v) => set("defaultOgDescription", v || null)}
          />
        </div>
      </section>
      ...
      <button
        ...
        className="bg-foreground text-primary-foreground px-6 py-3 text-[11px] tracking-wide disabled:opacity-50"
      >
        {busy ? "Saving..." : "Save Brand / SEO"}
      </button>
    </div>
  );
}
